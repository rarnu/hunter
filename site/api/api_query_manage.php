...
while (list($id, $jt, $wa, $cn)=mysql_fetch_row($result)) {
	if ($from == "site") {
		$str = $str."<tr><td>$id</td><td>$jt</td><td>$wa</td><td>$cn</td><td><input type='button' value='修改' onclick='doEdit($id);' class='btn btn-primary'>&nbsp;&nbsp;<input type='button' value='属性' onclick='doProp($id);' class='btn btn-primary'>&nbsp;&nbsp;<input type='button' value='删除' onclick='doDelete($id);' class='btn btn-primary'></td></tr>";
	} else {
